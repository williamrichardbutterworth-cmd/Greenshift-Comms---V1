...
- **LOA spacing + "what they buy" relocation** (web-only, gates green, browser-verified) — (1) LOA page-1 value column nudged off the cell divider: measured divider `x≈200.6`→cell-right `≈538.8` via pymupdf, moved values `x=200`→**`x=212`** (~11pt left padding, matching the pre-printed text) + capped the wide fields' `maxWidth` to stay inside the right border. `LOA_FIELD_POS` (one map → editor + fill). (2) **Removed the Services chips** entirely (`CustomerVariables` is now `{fuel}` only; deleted `CustomerVariablesEditor.tsx`). (3) **"What they buy" (fuel) folded into the client record** — shown in `ClientProfilePanel` (read pill + Electric/Gas/Both toggle in Edit&add), still auto-extracted by the intake (`mergeIntakeIntoInputs` → `customerVariables.fuel`); removed the standalone hub card. **No schema change.**
