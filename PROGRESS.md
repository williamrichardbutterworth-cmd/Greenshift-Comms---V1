...
- **Deferred**: Phase 2 **Procure-Ahead** (forward curve + live market cards + generated 12-month chart, tailored to contract end + curve signal); Phase 3 **email interlink** + remove the now-dead server `/api/report/{draft,assemble,edit}` + `templatesStore`/`routes/templates` + legacy projects in the recent-docs list.
