...
- **LOA PDF fill**: `fillLoaPdf` (`lib/loa.ts`) overlays text on the REAL stored template (`web/public/loa-template.pdf`) via pdf-lib at coordinates MEASURED from that exact 2-page A4 (595×842) template — value column x≈200 on page 1, signature positions on page 2, drawn at `y = 842 - (labelY + 9)`. These coords are template-specific: if the template PDF is swapped/edited the coordinates must be re-measured (pymupdf `get_text('words')`), else fields land wrong. LOA fields use empty-string === unknown; `deriveLoaFromClient` seeds from the profile without clobbering saved values.
